...
export class EvidenceGraphValidator {
  ...
  /**
   * Validates that an EvidenceNode's internal hash matches its observation.
   */
  ...
  /**
   * Strictly verifies graph causality and asserts that no node exists without prerequisite evidence.
   */
  public static assertValidGraph(graph: EvidenceGraph): void {
    ...
    for (const node of graph.nodes) {
      if (!this.verifyNodeHash(node)) {
        throw new EvidenceGraphIntegrityViolation(
          `Node ${node.id} has invalid/tampered evidenceHash`,
    ...
    // Edge validation & Reachability from INCIDENT
    ...
    // Invariant 1: ROOT_CAUSE_CONFIRMED requires a preceding SANDBOX node with VERIFIED status
    ...
    // Invariant 2: HITL requires ROOT_CAUSE_CONFIRMED
    ...
    // Invariant 3: REMEDIATION requires HITL with VERIFIED status
    ...
    // Invariant 4: RECOVERY requires REMEDIATION and independent PROMETHEUS_MCP post-query
